...
class ZAGRNN(Classifier):
    ...
    def __init__(self, dataset, config):
        ...
        self.rnn = RNN(
            ...
            num_layers=config.ZLWARNN.num_layers, batch_first=True,
            bidirectional=config.ZLWARNN.bidirectional,
            rnn_type=config.ZLWARNN.rnn_type)

        self.label_wise_attention = LabelWiseAttention(
            feat_dim=config.ZLWARNN.hidden_dimension*2 if config.ZLWARNN.bidirectional else config.ZLWARNN.hidden_dimension,
            label_emb_dim=config.label_embedding.dimension,
            store_attention_score=config.ZLWARNN.store_attention_score)

        if config.ZLWARNN.use_gcn:
            self.gcn = torch.nn.ModuleList([
                GraphConvolution(
                    in_features=config.ZLWARNN.gcn_in_features,
                    out_features=config.ZLWARNN.gcn_hidden_features,
                    bias=True,
                    act=torch.relu_,
                    featureless=False,
                    dropout=config.ZLWARNN.gcn_dropout),
                GraphConvolution(
                    in_features=config.ZLWARNN.gcn_hidden_features,
                    out_features=config.ZLWARNN.gcn_out_features,
                    bias=True,
                    act=torch.relu_,
                    featureless=False,
                    dropout=config.ZLWARNN.gcn_dropout)
            ...
            self.doc_out_transform = torch.nn.Sequential(
                torch.nn.Linear(
                    in_features=config.ZLWARNN.hidden_dimension*2 if config.ZLWARNN.bidirectional else config.ZLWARNN.hidden_dimension,
                    out_features=config.ZLWARNN.gcn_in_features + config.ZLWARNN.gcn_out_features
                ),
                torch.nn.ReLU()
            )
    ...
    def get_parameter_optimizer_dict(self):
        ...
        if self.config.ZLWARNN.use_gcn:
